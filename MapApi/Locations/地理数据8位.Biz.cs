...
public partial class Geo8 : Entity<Geo8>, IGeo
{
    ...
    public override void Valid(Boolean isNew)
    {
        ...
        if (Hash.IsNullOrEmpty()) Hash = GeoHash.Encode(Longitude, Latitude, 8);
    ...
    #region 业务操作
    public static Geo8 Upsert(GeoPoint point, GeoAddress geoAddress, GeoPoint bd09, GeoPoint gcj02, Int32 days)
    {
        var hash = GeoHash.Encode(point.Longitude, point.Latitude, 8);
        var g = GetOrAdd(hash, FindByHash, k =>
        {
            var gd = new Geo8 { Hash = k, CreateTime = DateTime.Now };
    ...
    public static Geo8 GetOrAdd(GeoPoint point)
    {
        var hash = GeoHash.Encode(point.Longitude, point.Latitude, 8);
        return GetOrAdd(hash);
    }

    public static Geo8 GetOrAdd(String hash) => GetOrAdd(hash, FindByHash, k => new Geo8 { Hash = k, CreateTime = DateTime.Now });
